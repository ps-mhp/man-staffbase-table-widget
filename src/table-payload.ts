 ...
 * can act on. It is also the encoding the widget SDK itself uses for
 * non-scalar attribute values (`data:text/plain;base64,…`), so structured
 * payloads travelling as opaque ASCII is the sanctioned shape on this
 * platform — this module just applies it explicitly instead of relying on the
 * SDK's automatic decoding, whose result type differs from a string.
 */
